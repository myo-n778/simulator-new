(function(){
  ...
  const SLIT_COUNT=10;
  const initial={spacing:6,observe:1,screen:'spherical',light:'single',color:'緑'},state={...initial};
  ...
  function update(){
    ...
    inputs.forEach(i=>{const k=i.dataset.key;if(k==='observe'){i.min=-max;i.max=max;}i.value=state[k];const unit={spacing:' μm',observe:' 次'}[k];if(unit)root.querySelector(`[data-out="${k}"]`).textContent=state[k].toFixed(k==='spacing'?1:0)+unit;});
