(function(){
  ...
  function numberField(key,label,unit,min,max,step){return `<div class="interference-field"><label>${label}<output data-out="${key}"></output></label><input type="number" data-key="${key}" aria-label="${label}" min="${min}" max="${max}" step="${step}" value="${initial[key]}"></div>`;}
  ...
  function singleSource(){if(state.color!=='custom')return COLORS.find(c=>c.name===state.color)||COLORS[3];const visible=state.wavelength>=380&&state.wavelength<=780;return {name:visible?'任意の単色光':'可視域外の単色光',wavelength:state.wavelength,color:visible?spectralColor(state.wavelength):(state.wavelength<380?'#a78bfa':'#fb7185')};}
  ...
  function applyControl(i){const key=i.dataset.key;if(key==='wavelength'){const value=Number(i.value);state.wavelength=Number.isFinite(value)?Math.max(100,Math.min(2000,value)):state.wavelength;state.color='custom';}else state[key]=i.type==='range'?Number(i.value):i.value;if(key==='color'&&state.color!=='custom')state.wavelength=singleSource().wavelength;update();draw();}
